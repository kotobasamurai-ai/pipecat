...
Tests cover:
- Cache miss → stores audio, cache hit → replays with correct context_id
...
# Insert the external package so imports work without installation
sys.path.insert(0, "/tmp/pipecat-tts-cache/src")
...
class TestTTSCacheMixinIntegration:
    ...
    async def test_cached_frames_have_context_id(self, backend):
        ...
        # Cache replay only yields TTSAudioRawFrame(s) — no Started/Stopped
        # (those are managed by the base class)
    ...
    @pytest.mark.asyncio
    async def test_replayed_frames_are_audio_only(self, backend):
        """Cache replay should yield only TTSAudioRawFrame(s).

        TTSStartedFrame and TTSStoppedFrame are managed by the base TTSService
        (via push_start_frame, stop-frame handler, audio-context teardown).
        The mixin must NOT yield them to avoid duplicates.
        """
        svc = self._make_service(backend)
        ...
        for f in replayed:
            assert isinstance(f, TTSAudioRawFrame), (
                f"Expected only TTSAudioRawFrame from cache replay, got {type(f).__name__}"
            )
    ...
    async def test_replayed_frames_marked_as_from_cache(self, backend):
        """Replayed audio frames should have the _tts_cache_origin attribute set."""
        svc = self._make_service(backend)
...
class TestWordTimestampCaching:
    ...
    async def test_word_timestamps_collected_in_batch(self, backend):
        ...
        assert len(svc._batch_word_timestamps) == 2
        assert svc._batch_word_timestamps[0] == ("hello", 0.0)
        assert svc._batch_word_timestamps[1] == ("world", 0.5)
...
class TestBatchFinalization:
    ...
    async def test_multiple_tasks_without_timestamps_skipped(self, backend):
        ...
        svc._batch_cache_tasks.append(BatchCacheTask(text="first", cache_key="key1", word_count=1))
        svc._batch_cache_tasks.append(BatchCacheTask(text="second", cache_key="key2", word_count=1))
        svc._batch_audio_buffer.append(
            CachedAudioChunk(audio=_FAKE_AUDIO, sample_rate=16000, num_channels=1)
        )
...
# ---------------------------------------------------------------------------
# Tests: Metrics compatibility
# ---------------------------------------------------------------------------


class TestMetricsCompatibility:
    """Test that cache hits don't pollute pipeline metrics."""
    ...
    async def test_usage_metrics_skipped_on_cache_hit(self, backend):
        ...
        try:
            ...
            assert len(usage_calls) == initial_count, (
                f"Usage metrics should not fire on cache hit, but got {len(usage_calls) - initial_count} extra calls"
            )
        finally:
